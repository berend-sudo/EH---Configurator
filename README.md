...
| `/api/configurator/submit` | Renders the PDF, emails it, logs the lead to Sheets.   |

The "Continue to summary →" button on the configurator carries the
selection (`?bedrooms=&roof=&budget=&delta=`) through to `/summary`.
...
full name, email, phone, *"Intended timeline to delivery"*, and a
consent paragraph linking the terms & privacy PDFs. The **Generate
PDF** button stays `disabled` until every field is valid and consent
is ticked (`canGenerate` in `src/app/summary/page.tsx`).
...
3. **Logs the lead** to the *"EH Configurator leads"* Google Sheet
   (`src/lib/server/sheets.ts`, service-account JWT + Sheets API v4).
   Best-effort: a sheet failure is logged but never blocks the email.

The `EH-YYYY-NNNN` reference is generated client-side and carried
through the payload, so it's identical on the PDF cover, the email,
and the sheet row. Output filenames flow through `dxfFilename` /
`pdfFilename` in `src/lib/design-id.ts` — the single source of truth
for the `EH_<TYP>[-<SUB>]_<BR>BR_v<n>` scheme.
...
| `GOOGLE_SHEETS_SERVICE_ACCOUNT_JSON` | Service-account JSON (shared into the sheet) for the lead log. |
...
in front of `sendDesignEmail` if delivery volume warrants it. Sheet
writes are fire-and-forget with error logging; the placeholder legal
PDFs under `public/legal/` are generated by
...
- `clientInfo: { name, email, phone, timeline, consent }` — local to
  the `/summary` screen; posted to `/api/configurator/submit` on
  Generate PDF.
